...
Route::get('/', function () {
    ...
    if (Auth::check()) {
        ...
        // Example trial days calculation
        // Replace with your actual logic to determine trial days
        $trialDays = $user->trial_ends_at ? $user->trial_ends_at : 0;
    }

    return view('welcome', compact('notifications', 'trialDays'));
});
...
Route::middleware('auth')->group(function () {
    ...
    //manage the location using the livewire and alpine
    ...
    Route::get('/locations', [SubscriptionController::class, 'manage'])->name('locations.manage');
...
Route::post('logout', [LoginController::class, 'logout'])->name('logout');
    ...
    // used routes for testing before using 
    ...
    /*  Route::get('/geto', function () {
        
        $users = User::with('locations')->get(); // Fetch users with their associated locations

        foreach ($users as $user) {
            foreach ($user->locations as $location) {

                // Get forecast for the location by city or coordinates
                if ($location->name) {
                    $forecast = $this->weatherService->getHourlyForecast($location->city);
                    dd($forecast);
                ...
                // Check if rain is expected at this location
